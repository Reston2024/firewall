...
**Success Criteria** (what must be TRUE):
  1. The triage systemd timer runs on schedule and the `triage-results-*` index in OpenSearch grows with new enriched alert documents after each execution
  2. A sample triage result document contains: original alert fields, ATT&CK technique ID + confidence, AI-generated summary, and retrieved RAG chunk references
  3. An OpenSearch Dashboards index pattern for `triage-results-*` is configured and an analyst can view enriched triage results without touching raw Malcolm network indices
  4. The triage worker completes a batch of 10 high-severity alerts without triggering an OOM kill (confirmed via `dmesg | grep -i oom` showing no events during triage run)
**Plans**: TBD
