#!/bin/bash
# rollback-syslog.sh — Restore syslog config from backup
# Usage: bash rollback-syslog.sh [backup-file]
...
echo "Config restored. Reloading syslog daemon..."
/etc/init.d/sysklogd restart
if [ $? -ne 0 ]; then
  echo "ERROR: sysklogd restart failed. Config may be invalid."
  exit 1
fi

echo "Rollback complete. Syslog daemon reloaded successfully."
echo "Verify: tail -5 /var/log/messages"
exit 0
